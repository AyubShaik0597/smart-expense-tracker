...
- **Validation**: `ExpenseRequest` uses Bean Validation (`@NotBlank`,
  `@Positive`, etc.) so bad input is rejected with a 400 and a clear message
  before it ever reaches the service layer.
- **Error handling**: a `@RestControllerAdvice` (`GlobalExceptionHandler`)
  turns "expense not found" and validation failures into consistent JSON
  error bodies instead of raw stack traces.
- **Category matching** is case-insensitive (`"food"` matches `"Food"`) since
  that's how a real user would expect filtering to behave.
- **Thread safety**: the repository uses `ConcurrentHashMap` +
  `AtomicLong` for id generation, so concurrent requests won't corrupt state,
  even though a single-user take-home assignment is unlikely to need it.
